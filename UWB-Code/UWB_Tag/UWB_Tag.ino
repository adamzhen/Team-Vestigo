...
void loop() 
{
  ...
  while(looping)
  {
     // Determine the number of anchors to range with
    int numAnchors = (topSixCycles < 2) ? 6 : keys.size();

    // Prioritize most responsive anchors for ranging
    ...
    // If we've just completed a full cycle through the top six anchors, increment the counter
    if (numAnchors == 6) {
      ++topSixCycles;
    }

    // Existing code to process the ranging results and check for termination condition
    for (const auto& key : keys) 
    {
      float distance = 0;
      double tof = 0;
